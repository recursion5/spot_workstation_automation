...
8. **Manageable after the USB build (2026-08-27).** Every replacement (SPOT and non-SPOT) finishes with **WinRM** the same way discovery works today: HTTP **5985**, NTLM, `LocalAccountTokenFilterPolicy=1`, firewall to controller **`10.0.253.225/32`**, **`ZenithAdmin` only**. The USB applies `Bootstrap-WinRM.ps1` (or equivalent); the operator does not run it by hand. That is what lets this controller / later agents reach the new box. Ansible is **not** a Windows agent and is **not** required on the stick; it remains optional controller-side orchestration over WinRM. Do not add OpenSSH, Intune, Salt, or a second remote-management stack unless a later decision.
