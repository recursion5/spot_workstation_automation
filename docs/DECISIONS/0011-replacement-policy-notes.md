...
2. **RustDesk.** Installed on replacements, ID/rendezvous/relay server **`rustdesk.vogueclean.int`** (DNS name, not a raw IP). Live Zenith PCs still use `10.0.253.110` (same address as `zenith-dsm.vogueclean.int`). **2026-08-27:** operator corrected the hostname from `dsm.vogueclean.int`. From the controller, `rustdesk.vogueclean.int` did **not** resolve that day; do not point live configs at it until DNS exists.
