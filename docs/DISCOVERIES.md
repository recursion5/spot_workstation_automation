...
- **PUP cleanup 2026-08-22 (operator-requested):** Gayla profile had **OneLaunch 5.42** (startup updater + tray), **OneBrowser** (scheduled task `OBUpdate` running), desktop/Start Menu shortcuts, and Chrome extension **Cash Catch** (`cjbmfmeflcomeifhpeglfmpgmmhcopdo`). Removed those. Start page still hijacked via Chrome **Secure Preferences**: default search labeled Yahoo but URL `velis-browser.com`, plus `new_tab_url` / `homepage` / `startup_urls` on that host. Replaced those URLs; machine policy now forces Google search and New Tab on startup (`RestoreOnStartup=5`). Left 3CX Click to Call, Adobe Acrobat, Google Docs Offline. Yevhen account still on disk (not deleted).
