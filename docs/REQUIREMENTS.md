...
- **Secrets (2026-08-27):** pack root `\\zenith-dsm.vogueclean.int\spot-rebuild`, SMB user **`spot-rebuild`** (ADR-0013). Password not in git. Vendor kits copied to `common/packages/`.
...
- **Post-build remoting (2026-08-27):** every replacement is reachable from this controller the same way as discovery: WinRM HTTP 5985 NTLM, firewall `10.0.253.225/32`, admin `ZenithAdmin` only. USB applies the bootstrap. Ansible stays on the Linux controller if used; it is not installed on the workstation. No extra Windows management agent unless later decided.
